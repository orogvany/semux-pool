...
public class PoolPayer
{
    ...
    private final String delegatePrivateKey;
    Long nonce = 0l;

    public PoolPayer(boolean signLocal, String delegatePrivateKey, SemuxClient client, String delegateAddress, Persistence persistence, long fee,
        ...
        for (Map.Entry<String, Long> pay : payout.getPayouts().entrySet())
        {
            ...
            try
            {
                ...
                } else if (dontPayPoolQuitters && !isCurrentVoter(pay.getKey(), currentVotes))
                {
                    ...
                    if (signLocal)
                    {
                        String raw = client.composeRawTransaction("MAINNET", "TRANSFER", fee, nonce, payQuitterAddress, amountToSend, System.currentTimeMillis(), note.getBytes());
                        String sig = client.signMessageLocal(delegatePrivateKey, raw);
                        confirmation = client.sendTransaction(raw, sig);
                        nonce++;
                ...
                } else
                {
                    if (signLocal)
                    {
                        String raw = client.composeRawTransaction("MAINNET", "TRANSFER", fee, nonce, pay.getKey(), amountToSend, System.currentTimeMillis(), note.getBytes());
                        String sig = client.signMessageLocal(delegatePrivateKey, raw);
                        confirmation = client.sendTransaction(raw, sig);
                        nonce++;
            ...
            } catch (SemuxException | InvalidKeySpecException e)
            {
